...
export const CreateEventModal: React.FC<CreateEventModalProps> = ({
    ...
    try {
      ...
      const res = await eventsApi.create({
        ...
        barcode_config: {
          mode: 'prefix',
          value: '',
          identifier_field: primaryScanField,
  ...
  return createPortal(
    ...
    >
      <div
        className="relative bg-[#1B303A] border-t sm:border border-[#314A56] rounded-t-2xl sm:rounded-2xl max-w-2xl w-full max-h-[92vh] sm:max-h-[88vh] flex flex-col shadow-2xl shadow-black/80 overflow-hidden sm:my-auto animate-in slide-in-from-bottom-6 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Mobile Swipe/Pull Indicator */}
        <div className="pt-2.5 pb-1 sm:hidden flex justify-center bg-[#1B303A]">
          <div className="w-12 h-1 rounded-full bg-[#314A56]" />
        </div>

        {/* Modal Header */}
        <div className="flex-shrink-0 px-5 sm:px-6 py-4 sm:py-5 border-b border-[#314A56] flex items-start justify-between gap-3 bg-[#1B303A]">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <h2
                id="create-event-modal-title"
                className="text-lg sm:text-xl font-bold text-[#ECEEF0] tracking-tight"
              >
                {step === 'FORM' ? 'Create New Event' : 'Review & Confirm Event'}
              </h2>
              <span className="text-[10px] sm:text-xs font-semibold px-2 py-0.5 rounded-md bg-[#0C1B23] text-[#FFE3A6] border border-[#314A56] font-mono">
          ...
          <button
            ...
            className="p-2 rounded-lg text-[#8A9BA8] hover:text-[#ECEEF0] hover:bg-[#314A56]/30 border border-transparent hover:border-[#314A56] transition-all cursor-pointer -mr-1 -mt-1 shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-shrink-0 px-5 sm:px-6 py-4 border-t border-[#314A56] bg-[#1B303A] flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between sm:justify-end gap-2.5 sm:gap-3">
          {step === 'FORM' ? (
            <>
              ...
              <button
                ...
                className="w-full sm:w-auto px-4 py-2.5 rounded-xl text-sm font-medium text-[#8A9BA8] hover:text-[#ECEEF0] bg-[#0C1B23] hover:bg-[#314A56]/40 border border-[#314A56] transition-colors cursor-pointer disabled:opacity-50 text-center"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isSubmitting}
                className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-sm font-bold text-[#10232D] bg-[#FFE3A6] hover:bg-[#ffeac0] active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
              >
                <span>Finish</span>
                <ArrowRight className="w-4 h-4 text-[#10232D]" />
              </button>
            </>
          ) : (
            <>
              ...
              <button
                ...
                disabled={isSubmitting}
                className="w-full sm:w-auto px-4 py-2.5 rounded-xl text-sm font-medium text-[#8A9BA8] hover:text-[#ECEEF0] bg-[#0C1B23] hover:bg-[#314A56]/40 border border-[#314A56] transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
              >
                <Pencil className="w-4 h-4 text-[#8A9BA8]" />
                <span>Edit</span>
              </button>
              <button
                type="button"
                ...
                disabled={isSubmitting}
                className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-sm font-bold text-[#10232D] bg-[#FFE3A6] hover:bg-[#ffeac0] active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin text-[#10232D]" />
                    <span>Saving Event...</span>
                  </>
                ) : (
                  <>
                    <Check className="w-4 h-4 stroke-[2.5] text-[#10232D]" />
                    <span>Save</span>
                  </>
                )}
              </button>
